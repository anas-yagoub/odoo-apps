...
class Contract(models.Model):
    ...
    currency_ids = fields.Many2one('res.currency', related='company_id.currency_id', readonly=False,
                                    string='Currencyss', default=lambda self: self.env.company.currency_id)
    symbol = fields.Many2one(string="symbol", related='company_id.currency_id', readonly=False)
    rate = fields.Many2one('res.currency.rate', string="Currency", readonly=False)
    ...
    # def _taken_wage(self):
    #     for r in self:
    #         if r.currency_id == 'USD':
    #             r.wage = r.currency_id * r.rate
